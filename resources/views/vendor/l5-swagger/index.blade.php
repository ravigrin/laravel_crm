...
<script>
    window.onload = function() {
        ...
        @foreach($urlsToDocs as $title => $url)
            @php
                // Fix URL: convert query parameter (?api-docs.json) to path parameter (/api-docs.json)
                $fixedUrl = str_replace('?', '/', $url);
            @endphp
        ...
        const ui = SwaggerUIBundle({
            dom_id: '#swagger-ui',
            @if(count($urlsToDocs) === 1)
            @php
                $singleUrl = str_replace('?', '/', reset($urlsToDocs));
            @endphp
            url: "{{ $singleUrl }}",
            @else
            urls: urls,
